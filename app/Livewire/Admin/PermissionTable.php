    ...
    public function actions(Permission $row): array
    {
        ...
        if ($user && $user->can('permisos.ver')) {
            $buttons[] = Button::add('show')
                ...
                ->route('system.permissions.show', ['permission' => $row->id]);
        }
        //Solo mostrar si el usuario tiene permiso de editar
        if ($user && $user->can('permisos.editar')) {
            $buttons[] = Button::add('edit')
                ...
                ->route('system.permissions.edit', ['permission' => $row->id]);
        }
        // Solo mostrar si el usuario tiene permiso para eliminar
        if ($user && $user->can('permisos.eliminar')) {
            $buttons[] =  Button::add('destroy')
                ->id()
                ->class('relative group py-1.5 cursor-pointer hover:-translate-y-px bg-clip-text')
                ->attributes([
                    'onclick' => "openModal('delete-record', {$row->id}, '" . route('system.permissions.destroy', $row->id) . "')",
                    'type'    => 'button',
                ])
                ->slot(
