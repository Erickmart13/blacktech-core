<x-app-layout>
    ...
    <div class="border-2 p-5 md:p-2 lg:mx-5 shadow-xl">
        <form method="POST" action="{{ route('admin.roles.update', $role->id) }}" class=" flex flex-col min-w-0">
            @csrf
            @method('PUT')
            <div>
                @if ($errors->any())
            ...
            <div class="flex flex-wrap">
                <div class="mb-3 w-full max-w-full md:px-3 shrink-0  md:w-4/12 md:flex-0 md:mt-5">
                    ...
                    <input id="name" name="name" value="{{ old('name', $role) }}" type="text"
                ...
                <div class="mb-3 w-full max-w-full md:px-3 shrink-0 md:w-2/12 md:flex-0 md:mt-5">
                    ...
                    <select id="is_active" name="is_active" class="border rounded p-2 w-full">
                        <option value="1" {{ old('is_active', $role->is_active) == 1 ? 'selected' : '' }}>
                            Activo</option>
                        <option value="0" {{ old('is_active', $role->is_active) == 0 ? 'selected' : '' }}>
                            Inactivo</option>
                    </select>
                    @error('is_active')
                        <span class="text-red-500">{{ $message }}</span>
                                    ...
                                    @foreach ($perms as $permission)
                                        ...
                                        <label class="flex items-center gap-2 text-sm">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}"
                                                class="check-permission rounded" @checked($role->hasPermissionTo($permission->name))>
                                            {{ ucfirst($action) }}
                                        </label>
                                    @endforeach
                                </div>
