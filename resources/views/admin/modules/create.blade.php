<x-app-layout>
    ...
    <div class="border-2 p-5 md:p-2 lg:mx-5 shadow-xl">
        <form method="POST" action="{{ route('roles.store') }}" class=" flex flex-col min-w-0">
            @csrf
            <div>
                @if ($errors->any())
            ...
            <div class="flex flex-wrap">
                                    ...
                                    @foreach ($perms as $doc)
                                        <label class="flex items-center space-x-2">
                                            <input type="checkbox" name="permissions[]" value="{{ $doc->id }}"
                                                @checked(in_array($doc->id, old('permissions', [])))
                                                class="rounded border-gray-300 text-indigo-500 shadow-sm focus:ring-blue-600">
                                            <span>{{ $doc->name }}</span>
                                        </label>
                                    @endforeach
                ...
                <div class="mb-3 md:px-3 md:mt-5">
                    <x-button-save class="text-ellipsis">Guardar</x-button-save>
                </div>
            </div>
        </form>
    </div>
